...
void CANFD_Start( uint8_t id)
{
  //mapper(bruh, bruhhh, mapp);

  uint32_t Mask = 0x10 * (uint32_t)id;
  ...
  sFilterConfig.FilterID1 = Mask + 0x08;
  sFilterConfig.FilterID2 = Mask + 0x10;
  if (HAL_FDCAN_ConfigFilter(&hfdcan1, &sFilterConfig) != HAL_OK)
  {
    Error_Handler();
  }

  // Filter for broadcast messages from master.
  sFilterConfig.IdType = FDCAN_STANDARD_ID;
  sFilterConfig.FilterIndex = 0;
  sFilterConfig.FilterType = FDCAN_FILTER_RANGE;
  sFilterConfig.FilterConfig = FDCAN_FILTER_TO_RXFIFO0;
  sFilterConfig.FilterID1 = 0x08;
